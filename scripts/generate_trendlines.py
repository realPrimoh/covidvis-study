...
def add_image_col_to_df_with_date(state_cases_df, start_date, end_date=None):
    if end_date:
        ...
        state_image = state_cases_df.copy()
        state_image["image_url"] = "" # Will automatically fill up all columns
        state_image.loc[state_cases_df['Date'] == date, "image_url"] = "https://raw.githubusercontent.com/realPrimoh/covidvis-study/master/close-image.png"
...
# Function below generates interactive brush selection chart for rolling cases
    ...
    mean_bars_total = alt.Chart(df).mark_bar().encode(
        alt.X('Province_State:N', axis=alt.Axis(title="State")),
        alt.Y('mean(New_Cases_Rolling):Q', axis=alt.Axis(title='Average Number of COVID-19 Cases per Day in Entire Chart'),
              scale=alt.Scale(domain=(0, max(df['New_Cases_Rolling'])))),
        opacity=alt.value(0.9),
